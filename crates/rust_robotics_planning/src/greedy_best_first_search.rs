...
//!
//! Grid-based path planning using Greedy Best-First Search.
//! Unlike A*, the priority is based solely on the heuristic h(n),
//! ignoring the accumulated cost g(n). This makes it faster in many
//! cases but does NOT guarantee the shortest path.
//!
...
#[derive(Debug, Clone)]
pub struct GreedyBestFirstConfig {
...
impl Default for GreedyBestFirstConfig {
    fn default() -> Self {
        Self {
            resolution: 1.0,
            robot_radius: 0.5,
...
/// Node with priority for Greedy Best-First open set (min-heap)
#[derive(Debug)]
struct PriorityNode {
    x: i32,
    y: i32,
...
pub struct GreedyBestFirstPlanner {
    grid_map: GridMap,
    #[allow(dead_code)]
...
impl GreedyBestFirstPlanner {
    ...
    pub fn new(ox: &[f64], oy: &[f64], config: GreedyBestFirstConfig) -> Self {
        Self::try_new(ox, oy, config).expect(
            "invalid Greedy Best-First planner input: obstacle list must be non-empty and valid, and config values must be positive/finite",
        )
    }
    ...
    pub fn from_obstacles(ox: &[f64], oy: &[f64], resolution: f64, robot_radius: f64) -> Self {
        let config = GreedyBestFirstConfig {
            resolution,
            robot_radius,
        };
    ...
    fn calc_heuristic(n1_x: i32, n1_y: i32, n2_x: i32, n2_y: i32) -> f64 {
    ...
    fn plan_impl(&self, start: Point2D, goal: Point2D) -> RoboticsResult<Path2D> {
        ...
        open_set.push(PriorityNode {
            ...
            y: start_y,
            cost: 0.0,
            // Greedy Best-First: priority = h(n) only (no g-cost)
        ...
        while let Some(current) = open_set.pop() {
            ...
            for &(dx, dy, move_cost) in &self.motion {
                ...
                // Greedy Best-First: priority = h(n) only (no g-cost added)
                let priority = Self::calc_heuristic(new_x, new_y, goal_x, goal_y);
                open_set.push(PriorityNode {
                    x: new_x,
                    y: new_y,
                    cost: new_cost,
...
impl PathPlanner for GreedyBestFirstPlanner {
...
mod tests {
    ...
    #[test]
    fn test_greedy_best_first_finds_path() {
    ...
    #[test]
    fn test_greedy_best_first_from_obstacle_points() {
        let (ox, oy) = create_simple_obstacles();
        let obstacles = Obstacles::try_from_xy(&ox, &oy).unwrap();
        let planner = GreedyBestFirstPlanner::from_obstacle_points(
            &obstacles,
            GreedyBestFirstConfig::default(),
        )
        .unwrap();
    ...
    #[test]
    fn test_greedy_best_first_try_new_rejects_invalid_config() {
        let (ox, oy) = create_simple_obstacles();
        let config = GreedyBestFirstConfig {
            resolution: 0.0,
            ..Default::default()
        ...
        let err = match GreedyBestFirstPlanner::try_new(&ox, &oy, config) {
    ...
    #[test]
    fn test_greedy_best_first_preserves_asymmetric_world_coordinates() {
    ...
    #[test]
    fn test_greedy_best_first_path_not_necessarily_optimal() {
        // Greedy Best-First may find a longer path than A* due to
        // ignoring g-cost. We just verify it finds a valid path.
        let (ox, oy) = create_simple_obstacles();
        let planner = GreedyBestFirstPlanner::from_obstacles(&ox, &oy, 1.0, 0.5);

        let path = planner.plan_xy(2.0, 2.0, 8.0, 8.0).unwrap();

        // Path starts near start and ends near goal
        let first = path.points.first().unwrap();
        let last = path.points.last().unwrap();
