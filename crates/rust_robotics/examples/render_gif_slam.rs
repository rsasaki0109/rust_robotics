//! Render the SLAM demos (EKF-SLAM, FastSLAM 1.0, ICP) as animated GIFs.
//!
//! A robot loops through a landmark field with range-bearing observations.
//! Outputs: `media/gallery/ekf_slam.gif`, `media/gallery/fastslam.gif`,
//! and `media/gallery/icp_matching.gif`.
//!
//! ```bash
//! cargo run -p rust_robotics --example render_gif_slam --features "slam,gif"
//! ```

use nalgebra::{DMatrix, Vector2, Vector3};
use rand::{Rng, SeedableRng};
use rust_robotics::core::Pose2D;
use rust_robotics::slam::{
    ekf_slam::{ekf_slam_known_correspondences, EKFSLAMState},
    fastslam1::{create_particles, fastslam_update, get_best_particle},
    icp_matching::icp_matching,
};
use rust_robotics::viz::{colors, rgb, GifCanvasConfig, GifFrame, GifRecorder};

const DT: f64 = 0.1;
const STEPS: usize = 72;
const MAX_RANGE: f64 = 18.0;

const LANDMARKS: [[f64; 2]; 6] = [
    [2.5, 1.5],
    [6.0, 1.5],
    [9.5, 1.5],
    [2.5, 6.5],
    [6.0, 6.5],
    [9.5, 6.5],
];

const R_DIST: f64 = 0.3;
const R_ANGLE: f64 = 5.0 * std::f64::consts::PI / 180.0;

fn normalize_angle(angle: f64) -> f64 {
    let mut a = angle;
    while a > std::f64::consts::PI {
        a -= 2.0 * std::f64::consts::PI;
    }
    while a < -std::f64::consts::PI {
        a += 2.0 * std::f64::consts::PI;
    }
    a
}

fn motion_model(x: Vector3<f64>, u: Vector2<f64>) -> Vector3<f64> {
    Vector3::new(
        x[0] + u[0] * DT * x[2].cos(),
        x[1] + u[0] * DT * x[2].sin(),
        normalize_angle(x[2] + u[1] * DT),
    )
}

fn control(step: usize) -> Vector2<f64> {
    let phase = step % 36;
    if phase < 14 {
        Vector2::new(1.0, 0.0)
    } else if phase < 20 {
        Vector2::new(0.35, 0.55)
    } else if phase < 28 {
        Vector2::new(0.9, 0.0)
    } else {
        Vector2::new(0.35, 0.55)
    }
}

fn noisy_observations(rng: &mut rand::rngs::StdRng, pose: Vector3<f64>) -> Vec<(f64, f64, usize)> {
    let mut z = Vec::new();
    for (id, lm) in LANDMARKS.iter().enumerate() {
        let dx = lm[0] - pose[0];
        let dy = lm[1] - pose[1];
        let d = (dx * dx + dy * dy).sqrt();
        if d <= MAX_RANGE {
            let angle = normalize_angle(dy.atan2(dx) - pose[2]);
            let d_noisy = d + rng.sample(rand_distr::Normal::new(0.0, R_DIST).unwrap());
            let angle_noisy = angle + rng.sample(rand_distr::Normal::new(0.0, R_ANGLE).unwrap());
            z.push((d_noisy.max(0.05), angle_noisy, id));
        }
    }
    z
}

fn canvas() -> GifCanvasConfig {
    GifCanvasConfig::new(500, 400, (-2.0, 13.0), (-2.5, 9.5))
        .with_delay_cs(10)
        .with_grid_step(Some(5.0))
}

fn draw_landmarks(frame: &mut GifFrame) {
    for lm in LANDMARKS {
        frame.fill_circle(lm[0], lm[1], 0.14, (220, 180, 40));
        frame.draw_circle(lm[0], lm[1], 0.32, (220, 180, 40), 1.5);
    }
}

fn draw_common(
    frame: &mut GifFrame,
    truth_trail: &(Vec<f64>, Vec<f64>),
    truth: Vector3<f64>,
    est: Vector3<f64>,
    est_landmarks: &[[f64; 2]],
) {
    draw_landmarks(frame);
    for lm in est_landmarks {
        frame.draw_cross(lm[0], lm[1], rgb(colors::MEASUREMENT), 4.0);
    }
    frame.draw_path_xy(
        &truth_trail.0,
        &truth_trail.1,
        rgb(colors::GROUND_TRUTH),
        1.4,
    );
    frame.draw_robot(
        &Pose2D::new(truth[0], truth[1], truth[2]),
        0.35,
        rgb(colors::GROUND_TRUTH),
    );
    frame.draw_robot(
        &Pose2D::new(est[0], est[1], est[2]),
        0.35,
        rgb(colors::ESTIMATED),
    );
}

fn render_ekf_slam(rng: &mut rand::rngs::StdRng) {
    let output = "media/gallery/ekf_slam.gif";
    let cfg = canvas();
    let mut rec = GifRecorder::new(output, cfg.clone()).expect("create recorder");

    let mut true_pose = Vector3::zeros();
    let mut ekf = EKFSLAMState::new();
    let mut trail = (Vec::new(), Vec::new());

    for step in 0..STEPS {
        let u = control(step);
        let obs = noisy_observations(rng, true_pose);
        ekf_slam_known_correspondences(&mut ekf, &u, &obs, LANDMARKS.len());
        true_pose = motion_model(true_pose, u);
        trail.0.push(true_pose[0]);
        trail.1.push(true_pose[1]);

        let est = ekf.get_robot_pose();
        let est_landmarks: Vec<[f64; 2]> = (0..ekf.n_landmarks())
            .filter_map(|i| ekf.get_landmark(i).map(|lm| [lm[0], lm[1]]))
            .collect();

        let mut frame = GifFrame::new(&cfg);
        draw_common(
            &mut frame,
            &trail,
            true_pose,
            Vector3::new(est[0], est[1], est[2]),
            &est_landmarks,
        );
        let last = step + 1 == STEPS;
        rec.add_frame_with_delay(frame, if last { 150 } else { cfg.delay_cs })
            .expect("write frame");
    }

    rec.finish().expect("finalize gif");
    println!("Saved {}", output);
}

fn render_fastslam(rng: &mut rand::rngs::StdRng) {
    let output = "media/gallery/fastslam.gif";
    let cfg = canvas();
    let mut rec = GifRecorder::new(output, cfg.clone()).expect("create recorder");

    let mut true_pose = Vector3::zeros();
    let mut particles = create_particles(60, LANDMARKS.len());
    let mut trail = (Vec::new(), Vec::new());

    for step in 0..STEPS {
        let u = control(step);
        let obs = noisy_observations(rng, true_pose);
        fastslam_update(&mut particles, u, &obs);
        true_pose = motion_model(true_pose, u);
        trail.0.push(true_pose[0]);
        trail.1.push(true_pose[1]);

        let best = get_best_particle(&particles);
        let est_pose = Vector3::new(best.x, best.y, best.yaw);
        let est_landmarks: Vec<[f64; 2]> = best
            .landmarks
            .iter()
            .filter(|lm| lm.cov[(0, 0)] < 100.0)
            .map(|lm| [lm.x, lm.y])
            .collect();
        let cloud: Vec<[f64; 2]> = particles.iter().map(|p| [p.x, p.y]).collect();

        let mut frame = GifFrame::new(&cfg);
        for p in &cloud {
            frame.draw_point(p[0], p[1], (140, 190, 255), 1.4);
        }
        draw_common(&mut frame, &trail, true_pose, est_pose, &est_landmarks);
        let last = step + 1 == STEPS;
        rec.add_frame_with_delay(frame, if last { 150 } else { cfg.delay_cs })
            .expect("write frame");
    }

    rec.finish().expect("finalize gif");
    println!("Saved {}", output);
}

fn simulate_scan(rng: &mut rand::rngs::StdRng, pose: Vector3<f64>) -> Vec<[f64; 2]> {
    let mut pts = Vec::new();
    for lm in LANDMARKS {
        let dx = lm[0] - pose[0];
        let dy = lm[1] - pose[1];
        let d = (dx * dx + dy * dy).sqrt();
        if d <= MAX_RANGE && d > 0.4 {
            let nx = lm[0] + rng.sample(rand_distr::Normal::new(0.0, 0.04).unwrap());
            let ny = lm[1] + rng.sample(rand_distr::Normal::new(0.0, 0.04).unwrap());
            pts.push([nx, ny]);
        }
    }
    for deg in (0..360).step_by(15) {
        let ray = deg as f64 * std::f64::consts::PI / 180.0;
        let dir = Vector2::new(ray.cos(), ray.sin());
        let mut best = MAX_RANGE;
        for lm in LANDMARKS {
            let rel = Vector2::new(lm[0] - pose[0], lm[1] - pose[1]);
            let along = rel.dot(&dir);
            if along > 0.2 {
                let perp = (rel[0] * dir[1] - rel[1] * dir[0]).abs();
                if perp < 0.35 && along < best {
                    best = along;
                }
            }
        }
        if best < MAX_RANGE {
            let hit = Vector2::new(pose[0], pose[1]) + dir * best;
            let nx = hit[0] + rng.sample(rand_distr::Normal::new(0.0, 0.03).unwrap());
            let ny = hit[1] + rng.sample(rand_distr::Normal::new(0.0, 0.03).unwrap());
            pts.push([nx, ny]);
        }
    }
    pts
}

fn points_to_matrix(points: &[[f64; 2]]) -> DMatrix<f64> {
    let mut m = DMatrix::zeros(2, points.len());
    for (j, p) in points.iter().enumerate() {
        m[(0, j)] = p[0];
        m[(1, j)] = p[1];
    }
    m
}

fn render_icp(rng: &mut rand::rngs::StdRng) {
    let output = "media/gallery/icp_matching.gif";
    let cfg = canvas();
    let mut rec = GifRecorder::new(output, cfg.clone()).expect("create recorder");

    let mut true_pose = Vector3::zeros();
    let mut prev_scan = simulate_scan(rng, true_pose);
    let mut trail = (Vec::new(), Vec::new());

    for step in 0..STEPS {
        let u = control(step);
        true_pose = motion_model(true_pose, u);
        trail.0.push(true_pose[0]);
        trail.1.push(true_pose[1]);
        let curr_scan = simulate_scan(rng, true_pose);

        let aligned: Vec<[f64; 2]> = if prev_scan.len() >= 3 && curr_scan.len() >= 3 {
            let prev_mat = points_to_matrix(&prev_scan);
            let curr_mat = points_to_matrix(&curr_scan);
            let result = icp_matching(&prev_mat, &curr_mat);
            let rotated = &result.rotation * points_to_matrix(&curr_scan);
            curr_scan
                .iter()
                .enumerate()
                .map(|(j, _)| {
                    [
                        rotated[(0, j)] + result.translation[0],
                        rotated[(1, j)] + result.translation[1],
                    ]
                })
                .collect()
        } else {
            curr_scan.clone()
        };

        let mut frame = GifFrame::new(&cfg);
        draw_landmarks(&mut frame);
        for p in &prev_scan {
            frame.draw_point(p[0], p[1], (140, 190, 255), 2.0);
        }
        for p in &curr_scan {
            frame.draw_point(p[0], p[1], (255, 130, 130), 2.0);
        }
        for p in &aligned {
            frame.draw_point(p[0], p[1], (60, 190, 110), 2.4);
        }
        frame.draw_path_xy(&trail.0, &trail.1, rgb(colors::GROUND_TRUTH), 1.4);
        frame.draw_robot(
            &Pose2D::new(true_pose[0], true_pose[1], true_pose[2]),
            0.35,
            rgb(colors::GROUND_TRUTH),
        );
        let last = step + 1 == STEPS;
        rec.add_frame_with_delay(frame, if last { 150 } else { cfg.delay_cs })
            .expect("write frame");

        prev_scan = curr_scan;
    }

    rec.finish().expect("finalize gif");
    println!("Saved {}", output);
}

fn main() {
    let mut rng = rand::rngs::StdRng::seed_from_u64(42);
    render_ekf_slam(&mut rng);
    render_fastslam(&mut rng);
    render_icp(&mut rng);
}
