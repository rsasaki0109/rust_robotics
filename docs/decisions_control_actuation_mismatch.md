...
- Problem preset: `control-actuation-mismatch-windows`
...
- Keep `percentile-bucket` as the best current exploratory proxy. It reduces evaluation cost from `190.04 ms` to `100.13 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current control problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `18.86 ms`; its hard agreement with `full-bucket` is `0.778` and its near-tie-aware agreement is `0.667`.
