...
- Problem preset: `localization-process-noise-anisotropy-windows`
...
- Keep `first-scenario` as the best current exploratory proxy. It reduces evaluation cost from `4512.68 ms` to `449.12 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
