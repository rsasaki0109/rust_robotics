...
- Problem preset: `localization-long-horizon-windows`
...
- Keep `sampled-bucket` as the best current exploratory proxy. It reduces evaluation cost from `4810.27 ms` to `1417.57 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `475.98 ms`; its hard agreement with `full-bucket` is `0.889`, but its near-tie-aware agreement is `0.889`, so it should not drive filter-selection decisions.
...
- If the gap between `sampled-bucket` and `full-bucket` remains large, add another disposable variant instead of widening the stable surface.
