...
- Problem preset: `localization-process-mismatch-windows`
...
- Keep `first-scenario` as the best current exploratory proxy. It reduces evaluation cost from `4447.57 ms` to `448.84 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
