...
| B+ | ACLM distributed MPC | Code coming soon; initial slice implemented | Distributed MPC and payload transport | Centralized consensus-ADMM (`admm_consensus.rs`, box-constrained formation), decentralized edge-based graph consensus (`solve_graph_consensus`, `benchmark_admm_graph_consensus.rs`, line/ring/complete convergence-rate comparison), and receding-horizon trajectory consensus (`solve_horizon_consensus`, `benchmark_admm_horizon_consensus.rs`, banded Cholesky z-update + MPC loop over an L-corner) all done |
