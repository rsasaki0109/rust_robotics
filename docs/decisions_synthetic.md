...
- Problem preset: `synthetic-local-grids`
...
- Keep `first-scenario` as the best current exploratory proxy. It reduces evaluation cost from `68.56 ms` to `3.61 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current problem.
...
- Keep the interface stable and the implementations disposable. New variants must plug into the same input set (`3` families) and emit the same `RuntimeObservation` records.
