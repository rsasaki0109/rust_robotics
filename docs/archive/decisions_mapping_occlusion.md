...
- Problem preset: `mapping-occlusion-corruption-windows`
...
- Keep `variance-triggered` as the best current exploratory proxy. It reduces evaluation cost from `0.01 ms` to `0.02 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current mapping problem.
