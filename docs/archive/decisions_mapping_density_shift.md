...
- Problem preset: `mapping-density-shift-windows`
...
- Keep `variance-triggered` as the best current exploratory proxy. It reduces evaluation cost from `0.02 ms` to `0.02 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current mapping problem.
- Keep `sampled-bucket` as the cheapest smoke-test strategy. It runs in `0.01 ms`; its hard agreement with `full-bucket` is `1.000` and its near-tie-aware agreement is `0.889`.
