...
- Problem preset: `localization-dropout-bias-windows`
...
- Keep `percentile-bucket` as the best current exploratory proxy. It reduces evaluation cost from `3912.51 ms` to `1979.28 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `392.89 ms`; its hard agreement with `full-bucket` is `0.889`, but its near-tie-aware agreement is `1.000`, so it should not drive filter-selection decisions.
