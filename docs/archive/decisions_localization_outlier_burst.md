...
- Problem preset: `localization-outlier-burst-windows`
...
- Keep `variance-triggered` as the best current exploratory proxy. It reduces evaluation cost from `4279.09 ms` to `3093.58 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `431.68 ms`; its hard agreement with `full-bucket` is `0.778`, but its near-tie-aware agreement is `0.778`, so it should not drive filter-selection decisions.
