...
- Problem preset: `localization-control-latency-windows`
...
- Keep `variance-triggered` as the best current exploratory proxy. It reduces evaluation cost from `4678.75 ms` to `2434.12 ms`, while keeping hard agreement with `full-bucket` at `1.000` and near-tie-aware agreement at `1.000` on the current localization problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `447.99 ms`; its hard agreement with `full-bucket` is `0.778`, but its near-tie-aware agreement is `0.778`, so it should not drive filter-selection decisions.
...
- If the gap between `variance-triggered` and `full-bucket` remains large, add another disposable variant instead of widening the stable surface.
