...
- Problem preset: `localization-sensor-rate-mismatch-windows`
...
- Keep `percentile-bucket` as the best current exploratory proxy. It reduces evaluation cost from `4473.06 ms` to `2237.71 ms`, while keeping hard agreement with `full-bucket` at `0.889` and near-tie-aware agreement at `0.889` on the current localization problem.
- Keep `first-scenario` as the cheapest smoke-test strategy. It runs in `448.84 ms`; its hard agreement with `full-bucket` is `0.778`, but its near-tie-aware agreement is `0.667`, so it should not drive filter-selection decisions.
